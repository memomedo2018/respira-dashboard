...
RELEASE_DATA_PATHS = [
    ...
    BASE_DIR / "data" / "seo_gsc_query_pages.json",
    BASE_DIR / "data" / "seo_recommendations.json",
    BASE_DIR / "data" / "seo_indexing_queue.json",
    BASE_DIR / "data" / "seo_url_inspections.json",
    BASE_DIR / "data" / "seo_quality_reports.json",
    BASE_DIR / "data" / "seo_internal_links.json",
    BASE_DIR / "data" / "seo_content_refresh_queue.json",
    BASE_DIR / "data" / "seo_content_refresh_blocks.json",
    BASE_DIR / "data" / "seo_outreach_tasks.json",
    BASE_DIR / "data" / "seo_daily_reports.json",
    BASE_DIR / "data" / "seo_errors.json",
    STORE_FILE,
    SITE_FILE,
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .floating-header {{ position: fixed; top: 18px; left: 50%; transform: translateX(-50%); width: min(980px, calc(100vw - 2rem)); background: var(--theme-surface-strong); backdrop-filter: blur(18px); border: 1px solid var(--theme-border); border-radius: 999px; padding: .5rem .6rem .5rem .75rem; z-index: 1000; box-shadow: 0 18px 44px rgba(15,23,42,0.12); }}
    .header-content {{ min-height: 52px; display: flex; align-items: center; justify-content: space-between; gap: .8rem; }}
    .brand-logo {{ display: inline-flex; align-items: center; gap: .55rem; font-size: 1.05rem; font-weight: 900; color: var(--theme-accent); white-space: nowrap; }}
    .brand-mark {{ width: 34px; height: 34px; border-radius: 12px; object-fit: contain; }}
    .nav-links {{ display: flex; align-items: center; gap: .15rem; list-style: none; flex-wrap: nowrap; }}
    .nav-link {{ color: var(--theme-text-muted); font-size: .86rem; font-weight: 800; padding: .55rem .72rem; border-radius: 999px; transition: .25s ease; }}
    ...
    .header-cta {{ display: inline-flex; align-items: center; justify-content: center; min-height: 40px; padding: 0 .95rem; border-radius: 999px; background: #0f172a; color: #fff; font-size: .9rem; font-weight: 900; white-space: nowrap; transition: .2s ease; }}
    .header-cta:hover {{ transform: translateY(-1px); background: var(--theme-accent); }}
    ...
    @media (max-width: 768px) {{
      ...
      .header-content {{ min-height: 76px; position: relative; display: flex; flex-direction: row; direction: ltr; justify-content: space-between; align-items: center; gap: .75rem; }}
      .brand-logo {{ min-width: 0; font-size: 0; gap: 0; position: fixed; left: 88px; top: 32px; z-index: 1400; transform: none; }}
      .menu-toggle {{ position: fixed; left: 28px; top: 27px; z-index: 1400; transform: none; }}
      .brand-mark {{ width: 34px; height: 34px; }}
      .menu-toggle {{ display: inline-flex; }}
      .header-cta {{ display: none; }}
      .nav-links {{ display: none; position: fixed; inset: 0; z-index: -1; width: 100vw; min-height: 100svh; padding: 7.5rem 1.25rem 2rem; margin: 0; border-top: 0; background: rgba(255,255,255,.98); backdrop-filter: blur(14px); align-content: start; direction: rtl; }}
      body[data-theme="dark"] .nav-links {{ background: rgba(15,23,42,.98); }}
      .floating-header.is-open .nav-links {{ display: grid; gap: .75rem; }}
      .nav-link {{ display: block; width: 100%; text-align: center; padding: 1rem; font-size: 1.05rem; }}
...
<body class="{html.escape(body_class)}" data-theme="light">
  <header class="floating-header">
    <nav class="header-content">
      ...
      <a class="header-cta" href="https://wa.me/{html.escape(site['whatsapp_number'])}" target="_blank" rel="noopener">استشارة مجانية</a>
    </nav>
  </header>
